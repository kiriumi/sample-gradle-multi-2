...
* 公開Jarはサブプロジェクト単位
* Warはサブプロジェクト単位
* 業務バッチJarは共通あるいはサブシステムID.batch直下のパッケージ単位

## Gradleスクリプトの構成
* 全共通
* （Web／バッチ）×（ライブラリ／アプリケーション）
* 特殊①：公開Jar（Web）
* 特殊②：業務バッチ
...
* 社内Mavenリポジトリにアップロードする、公開Jar, 公開Jar（Web）のみバージョン番号を付ける
  ...
  * メジャー番号：機能追加・改修
  * マイナー番号：バグ改修
  * 単体テストのみ「-SNAPSHOT」、それ以外はなし
* 業務アーカイブには、案件名-YYYYMMDDhhmmssをバージョンとする
...
## DBマイグレーション
* flywayを使用
* src/db/migration にSQLファイルを格納
* SQLファイル名：R__{実行順}_{テーブル名}.sql
* 再実行可能なSQLにする
  * テーブル：CREATE TABLE IF NOT EXISTS {テーブル名}
  * データ：TRUNCATE -> INSERT
